...
function displayHourlyForecast(currentTime, data, length = 24) {
    ...
    for (let i = currentTime.getHours(); i < length + currentTime.getHours(); i++) {
        ...
        if ((new Date(data.daily.sunset[separatorCount]) < new Date(data.hourly.time[i]) && data.hourly.weather_code[i] === 0) || new Date(data.daily.sunrise[separatorCount]) > new Date(data.hourly.time[i]) && data.hourly.weather_code[i] === 0) {
            forecastItem.querySelector(".hourly-forecast-item__icon img").src = `graphics/weathers-icons/hours-light/clear-moon.svg`;
        } else {
            forecastItem.querySelector(".hourly-forecast-item__icon img").src = `graphics/weathers-icons/hours-light/${getWeatherConditionIcon(data.hourly.weather_code[i])}.svg`;
        }
...
function displayDailyWeather(data) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        forecastItem.querySelector(".daily-forecast-item__icon img").src = `graphics/weathers-icons/light-theme/${getWeatherConditionIcon(data["daily"]["weather_code"][i])}.svg`;
...
function switchHourlyForecastLength(length, index) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".segmented-button").forEach(segmentedButton => {
        ...
        segmentedButton.style.setProperty("--selected-width", `${buttons[0].offsetWidth}px`);
